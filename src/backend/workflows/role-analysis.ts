...
import { 
  roleBullets, roleBulletAnalyses, 
  roleResumeBullets, roleResumeBulletsMap, 
  roleBulletPatterns, roleBulletPatternMap 
} from "@/db/schema";
...
async function buildScoredBulletsContext(env: Env, roleId: string) {
  ...
  const analyses = await db.select().from(roleBulletAnalyses).where(
    inArray(roleBulletAnalyses.bulletId, bullets.map(b => b.id))
...
export class RoleAnalysisWorkflow extends WorkflowEntrypoint<Env, Params> {
  async run(event: WorkflowEvent<Params>, step: WorkflowStep) {
    ...
    await step.do("holistic-analysis", {
    ...
    await step.do("resume-bullet-ideation", {
      retries: { limit: 3, delay: "5 seconds" },
      timeout: "30 minutes",
    }, async () => {
      ...
      const inventoryContext = activeBullets.map((b) => `[${b.category}] ${b.content}`).join("\n");
      ...
      if (result.ideations.length > 0) {
        ...
        for (const ideation of result.ideations) {
          const [inserted] = await db.insert(roleResumeBullets).values({
            roleId,
    ...
    await step.do("pattern-recognition", {
      retries: { limit: 3, delay: "5 seconds" },
      timeout: "30 minutes",
    }, async () => {
      const db = getDb(this.env);
      ...
      if (result.patterns.length > 0) {
        ...
        for (const pattern of result.patterns) {
          const [inserted] = await db.insert(roleBulletPatterns).values({
            roleId,
    ...
    await step.do("notify-step-4", async () => await notifyAgent("pattern-recognition-complete", 100));
