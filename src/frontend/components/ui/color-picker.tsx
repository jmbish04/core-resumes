    ...
    return {
      ...
      setOpen: (value: boolean) => {
        ...
        if (propsRef.current.onOpenChange) {
          propsRef.current.onOpenChange(value, {} as any);
        }
...
function ColorPickerTrigger(props: ColorPickerTriggerProps) {
  const { asChild, disabled, render, children, ...triggerProps } = props;
  ...
  if (render) {
    return <PopoverTrigger disabled={isDisabled} render={render} {...triggerProps} />;
  }

  if (asChild && React.isValidElement(children)) {
    return <PopoverTrigger disabled={isDisabled} render={children as React.ReactElement} {...triggerProps} />;
  }

  return (
    <PopoverTrigger disabled={isDisabled} render={<Button data-slot="color-picker-trigger" {...(triggerProps as any)}>{children}</Button>} />
  );
}
