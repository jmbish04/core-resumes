...
> - Sources _can_ be deleted from NotebookLM, so the "Clawback Source" action will trigger a hard delete via the SDK (`client.sources.delete()`) and mark the blob inactive in D1.
