...
export interface JobBoardProvider {
  ...
  name: string;
  /** Display label: "Greenhouse" | "AshbyHQ" | "Gem" */
  displayName: string;
  /** Config key in health_check_config (e.g. "greenhouse_tokens") */
  healthConfigKey: string;
  /** True if this provider has a structured API (vs HTML scraping). */
  isApi: boolean;
...
The `company_job_board_defs` table must be pre-populated with all known provider definitions. This happens two ways:

#### A. Auto-seeding on promote (Component 4b above)

When a company is promoted and its system maps to a registered provider, the board def is auto-created if it doesn't exist. This is the **lazy** path.
...
Add board def seed data to the config defaults so the Config page can bootstrap them:

```ts
const JOB_BOARD_DEF_SEEDS = [
  { name: "Greenhouse", description: "Greenhouse Job Board API (boards-api.greenhouse.io)", isApi: true },
  { name: "AshbyHQ", description: "Ashby Posting API (api.ashbyhq.com)", isApi: true },
  { name: "Gem", description: "Gem Job Board API (api.gem.com)", isApi: true },
];
```

The config reset/seed route will upsert these into `company_job_board_defs` on initialization.

#### C. Onboarding new providers

When a new provider is added to the registry, its `displayName` and `isApi` flag are used to auto-create the board def row on first promote. The config seed list should also be updated for explicit initialization.
