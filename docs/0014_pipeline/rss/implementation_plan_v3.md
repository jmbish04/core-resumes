...
## Architecture — End-to-End Pipeline + HITL Flow
...
flowchart TD
    ...
    REL --> DB[("jobs_postings<br/>pipelineSource ✅<br/>location ✅<br/>isRecommended ✅")]

    DB --> DEDUP["R2 Dedup Catalog<br/>(persist indefinitely)"]

    subgraph "Discovery Scorer (4h Cron)"
        SCORER["isRelevantJob() batch<br/>keyword + location heuristic"]
    end

    DB --> SCORER
    SCORER -->|"sets isRecommended=true"| DB

    subgraph "HITL — Discovery Dashboard (/discovery)"
        TAB1["Tab 1: Analyzed Jobs<br/>isRecommended + analysisExecuted"]
        TAB2["Tab 2: Unanalyzed Queue<br/>isRecommended + NOT analyzed"]
        TAB3["Tab 3: Hot Companies<br/>api_companies.isRecommended"]
    end

    DB -->|"isRecommended jobs"| TAB2
    DB -->|"analyzed jobs"| TAB1
    DB -->|"recommended companies"| TAB3

    TAB2 -->|"Deep Analyze (AI)"| TAB1
    TAB1 -->|"Promote to Active Role"| ROLES["Active Roles<br/>(/roles)"]
    TAB3 -->|"Promote Company"| WATCH["Company Watch List<br/>(/companies)"]
```

### HITL Lifecycle

All pipelines (A, B, C, External) write to the **same `jobs_postings` table** with normalized `jobSiteId` and `pipelineSource`. The system then:

1. **Discovery Scorer** (runs on 4-hour cron) applies `isRelevantJob()` to unscored rows → sets `isRecommended = true` + `recommendationScore` + `recommendationReason`
2. **Recommended jobs** surface in the **Discovery Dashboard** (`/discovery`) under the "Unanalyzed Queue" tab
3. **User reviews** → clicks "Analyze Job" → triggers AI deep analysis (Kimi K2.5) → job moves to "Analyzed Jobs" tab with match score, verdict, salary extraction, requirement breakdown
4. **User promotes** → clicks "Promote to Core Applications" → seeds an active role in `/roles` with intake data, requirement bullets, and the full analysis snapshot
5. **Recommended companies** (companies with `isRecommended` jobs) appear in the "Hot Companies" tab → user can "Promote to Watch List" to add to Pipeline B's tracked companies for ongoing scanning

> [!NOTE]
> RSS-sourced jobs flow into this **exact same HITL funnel**. No special UI handling needed — the Discovery Dashboard already queries `jobs_postings` regardless of `pipelineSource`. The `pipelineSource` column is available for filtering if the user wants to view jobs by source in the future.
...
| HITL integration | RSS jobs enter the **same Discovery Dashboard** as Pipeline A/B — no separate UI needed |
